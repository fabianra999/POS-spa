<template>
  ...
  >
    <template v-slot:item.inventaryType="{ item }">
      {{ $filters.findByKey(listInventaryType, "id", item.inventaryType) }}
    </template>
              ...
              <v-form>
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-autocomplete
                        v-model="editedItem.inventaryType"
                        :label="$t('inventaryType')"
                        :items="listInventaryType"
                        item-title="name"
                        item-value="id"
                        required
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model.number="editedItem.location"
                        :label="$t('location')"
                        required
                        :error-messages="
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: "Id", key: "id" },
      { title: "Tipo de inventario", key: "inventaryType" },
  ...
  methods: {
    initialize({ page, itemsPerPage, sortBy }) {
      inventoryStore
        .GET_INVENTARIES({ page: page, perPage: itemsPerPage })
        .then(() => {
          this.dessertsUsers = inventoryStore.products.data;
          this.totalItems = inventoryStore.products.items;
        });
    },
